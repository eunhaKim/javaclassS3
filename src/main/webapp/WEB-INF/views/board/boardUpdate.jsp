...
<script>
  ...
  var chk_style = 'position:absolute;font-size:18px;line-height:1.2;font-weight:bold;border:0;border-radius:50%;'
	...
	$(()=>{ 
		if('${vo.FSName}' != '') {
			// 리스트 이미지가 있으면 attzone에 넣어주기 위한 부분
			let listImage = '${vo.FSName}'.split('/');
			for(i in listImage){
				$("#att_zone").append("<div style='"+div_style+"'><img src='"+listImage[i]+"' style='"+img_style+"'/><input type='button' value='X' onclick='delListImg("+i+")' style='"+chk_style+";background-color:#000;'/></div>");
			}
		}
	});
	
	function delListImg(i){
		let ans = confirm("기존 등록되어 있는 리스트 이미지를 삭제하시겠습니까??");
		if(!ans) return false;
		let listImage = '${vo.FSName}'.split('/');
		let img = listImage[i];
		console.log(img);
    ...
    fetch('${ctp}/board/boardUpdate?boardName=${boardName}&idx=${vo.idx}', {
        method: 'POST',
        body: formData
    }).then(response => {
        // 서버 응답 처리
      ...
      <div class="row g-4">
      	...
      	<div class="boardInput">
	        <form name="myform" id="myform" method="post" enctype="multipart/form-data">
	          ...
	          <div class="title">
	           <input type="text" name="title" id="title" value="${vo.title}" autofocus class="form-control mb-3" required />
	          </div>
	            ...
	            <textarea name="content" id="CKEDITOR" rows="6" class="form-control" required >${vo.content}</textarea>
			      ...
			      <div class="cont_btn text-center">
			      	<input type="button" value="수정" onclick="fCheck()" class="btn btn-primary mr-2"/>
